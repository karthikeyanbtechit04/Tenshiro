...
const getMockCourses = (role) => {
    ...
    for (let i = 1; i <= 120; i++) {
        ...
        courses.push({
            ...
            thumbnail: `https://ui-avatars.com/api/?name=${encodeURIComponent(difficulty + '+' + role)}&background=random&color=fff&size=400`,
            is_free: i % 4 === 0
        });
    }
    return courses;
...
exports.getPremiumOverview = async (req, res) => {
    const { roleTitle, domainName, language = 'en' } = req.body;
    ...
    try {
        ...
        const completion = await groq.chat.completions.create({
            ...
            messages: [
                {
                    role: "user",
                    content: `Generate a premium, executive-level Career Intelligence Report for the role: ${roleTitle} within the domain of ${domainName || 'General'}.
                    ...
                    Language: You MUST generate all text strictly in ${targetLang}.
        ...
        res.json({
            strategic_positioning: {
                headline: `System Orchestration & Enterprise ${domainName || roleTitle} Architecture.`,
...
exports.generateRoadmap = async (req, res) => {
    ...
    try {
        ...
        const completion = await groq.chat.completions.create({
            ...
            messages: [
                ...
                },
                {
                    ...
                    INSTRUCTIONS:
                    1. Divide the roadmap exactly into 1-month phases. If Total Duration is "X Months", generate exactly X phases.
                    2. Each phase must act as exactly 1 Month of the timeline. The SUM of all phase durations MUST EQUAL exactly ${duration}.
                    3. Each phase must contain:
                       - Title, duration (always "1 Month"), and color (hex code).
        ...
        // Dynamic Mock Generator based on duration, level, and goals
        const getMockRoadmap = (role, durationStr, userLevel) => {
            const roleLower = role.toLowerCase();
            // Parse duration (e.g., '3m' -> 3 months = 3 phases)
            const months = parseInt(durationStr) || 3;
            
            // User requested that the time duration directly dictates the phases: 1 Month = 1 Phase
            const numPhases = months; 

            const phases = [];

            // Define skill stacks for authentic mock data (extended for 12 months)
            const stacks = {
                frontend: {
                    skills: ['HTML5 & Semantic UI', 'CSS3 & Layout Architecture', 'Modern JavaScript (ES6+)', 'React.js & Components', 'React State Management', 'API Integration & Async', 'Frontend Routing', 'Testing (Jest/RTL)', 'Performance Optimization', 'Web Security', 'Next.js & SSR', 'Web3 / WASM'],
                    tools: ['VS Code', 'Chrome DevTools', 'Git/GitHub', 'NPM/Yarn', 'Vite', 'Postman', 'Vercel', 'Lighthouse', 'Figma', 'SonarQube', 'Docker', 'AWS']
                },
                backend: {
                    skills: ['Node.js Fundamentals', 'Express & Middleware', 'Database Modeling (SQL)', 'NoSQL Integrations', 'RESTful API Architecture', 'Authentication & JWT', 'Server-side Validations', 'Caching (Redis)', 'Message Queues', 'Microservices Basics', 'System Design', 'Serverless'],
                    tools: ['PostgreSQL', 'MongoDB', 'Insomnia', 'Docker', 'Redis', 'Nginx', 'PM2', 'Prometheus', 'RabbitMQ', 'Kubernetes', 'AWS', 'GCP']
                },
                data: {
                    skills: ['Python Basics', 'Pandas & Data Wrangling', 'Exploratory Data Analysis', 'Statistical Modeling Core', 'Machine Learning Basics', 'Supervised Learning', 'Unsupervised Learning', 'Deep Learning Basics', 'Natural Language Processing', 'Data Visualization', 'MLOps pipelines', 'Big Data Streaming'],
                    tools: ['Jupyter Notebooks', 'NumPy', 'Scikit-learn', 'TensorFlow', 'PyTorch', 'SQL', 'Tableau', 'PowerBI', 'HuggingFace', 'Airflow', 'Spark', 'Kafka']
                },
                fullstack: {
                    skills: ['Frontend Core', 'Backend Foundations', 'Database Access', 'Authentication Flow', 'State & Caching', 'Cloud Deployment', 'CI/CD Pipelines', 'System Scalability', 'Domain Driven Design', 'Realtime Systems', 'Containerization', 'Platform Engineering'],
                    tools: ['React', 'Node.js', 'PostgreSQL', 'Docker', 'GitLab CI', 'Terraform', 'Kubernetes', 'AWS', 'Sentry', 'WebSockets', 'New Relic', 'Datadog']
                },
                default: {
                    skills: [`${role} Principles`, `${role} Tooling`, 'Standard Procedures', 'Quality Assurance', 'Strategic Execution', 'Optimization Patterns', 'Advanced Methodology', 'Scaling Operations', 'Global Delivery', 'Team Leadership', 'Executive Reporting', 'Final Portfolio Review'],
                    tools: ['Industry IDE', 'Project Tools', 'Collaboration Platforms', 'Documentation', 'Version Control', 'Dashboards', 'Cloud Infra', 'Automation Kits', 'AI Assistants', 'CRM', 'ERP', 'Portfolio Builder']
                }
            };
            ...
            const isAdvanced = userLevel?.toLowerCase() === 'advanced';

            // Base titles tailored for 12 full months of progression
            let titles = [
                'Foundations', 'Core Concepts', 'Intermediate Elements', 'Advanced Paradigms', 
                'Practical Integrations', 'Domain Specialization', 'Ecosystem Deep Dive', 
                'Production Level Practices', 'Systems Optimization', 'Cloud Architecture', 
                'Team & Operations', 'Industry Readiness'
            ];

            if (isAdvanced) {
                titles = [
                    'Architecture Review', 'High-Performance Patterns', 'Enterprise Operations', 
                    'Advanced Security', 'Domain Microservices', 'Systems Interoperability', 
                    'Infrastructure as Code', 'Reliability Engineering', 'Advanced Automation', 
                    'Strategic Leadership', 'Global Deployments', 'Executive Technical Vision'
                ];
            }

            const colors = ['#6366f1', '#8b5cf6', '#06b6d4', '#10b981', '#f59e0b', '#ec4899', '#f43f5e', '#3b82f6', '#14b8a6', '#8b5cf6', '#eab308', '#ec4899'];

            const mockCourses = getMockCourses(role);
            const mockProjects = getDomainProjects(1, role);

            // If advanced, skip the first basic skills where possible
            const skillOffset = isAdvanced && selectedStack.skills.length > 5 ? Math.floor(selectedStack.skills.length / 3) : 0;

            for (let i = 0; i < numPhases; i++) {
                const step = i % 12; // Wrap just in case
                const skillIndex = (step + skillOffset) % selectedStack.skills.length;
                const phaseSkills = [selectedStack.skills[skillIndex]];
                const phaseTools = [selectedStack.tools[skillIndex]];
                
                phases.push({
                    id: i + 1,
                    phase: `Month ${i + 1}`,
                    title: titles[step] || `Module ${i + 1}`,
                    description: `Dedicating this month to mastering ${phaseSkills[0]} towards ${userLevel} tier ${role} capabilities.`,
                    duration: '1 Month',
                    color: colors[step % colors.length],
                    skills: [...phaseSkills, ...phaseTools],
                    courses: mockCourses.slice(i * 2, (i * 2) + 2).map(c => ({
                        name: c.title.replace('Beginner', userLevel || 'Beginner'),
                        platform: c.platform,
                        link: c.link,
                        duration: '2-4 Weeks',
                    ...
                    tasks: [
                        { id: 1000 + i * 10 + 1, title: `Architect ${phaseTools[0]} solutions`, description: `Apply structured design patterns to ${phaseTools[0]} workflows` },
                        { id: 1000 + i * 10 + 2, title: `Implement ${phaseSkills[0]}`, description: `Practical 4-week module covering ${phaseSkills[0]} from scratch.` }
                    ],
                    internship: i === numPhases - 1 ? { type: 'Job Placement Focus', focus: `${userLevel} Final Portfolio & Interviews`, where: 'Global Tech Market' } : null
                });
            }
        ...
        res.json(getMockRoadmap(role, duration, level));
    }
};

// Save a roadmap for the user
exports.saveUserRoadmap = async (req, res) => {
    const { roleTitle, roadmapData } = req.body;
    const userId = req.user.id;

    if (!roleTitle || !roadmapData) {
        return res.status(400).json({ message: 'Role title and roadmap data are required' });
    }

    try {
        // Deactivate old roadmaps for this user
        await db.query('UPDATE user_roadmaps SET is_active = FALSE WHERE user_id = ?', [userId]);

        // Insert new one
        await db.query(
            'INSERT INTO user_roadmaps (user_id, role_title, roadmap_data) VALUES (?, ?, ?)',
            [userId, roleTitle, JSON.stringify(roadmapData)]
        );
