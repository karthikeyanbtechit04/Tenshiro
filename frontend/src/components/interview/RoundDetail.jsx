...
const RoundDetail = ({ selection, round, onBack }) => {
    ...
    const sections = [
        ...
        { id: 'learning', name: `C) ${t('interview.learningSection')}`, icon: <FaBookOpen />, hideOnMobile: true },
        { id: 'practice', name: `D) ${t('interview.practiceSection')}`, icon: <FaCode /> },
        { id: 'resources', name: `E) ${t('interview.resourcesSection')}`, icon: <FaLink />, hideOnMobile: true },
        { id: 'strategy', name: `F) ${t('interview.strategySection')}`, icon: <FaLightbulb />, hideOnMobile: true },
    ...
    const calculateActualScore = () => {
        if (!data || !data.readinessTest) return;
        
        if (Object.keys(answers).length < data.readinessTest.length) {
            alert("Please answer all questions before calculating your score.");
            return;
        }

        let correct = 0;
        data.readinessTest.forEach((q, i) => {
            if (answers[i] === q.correct) correct++;
        });
        const percentage = Math.round((correct / data.readinessTest.length) * 100);
                ...
                <aside className="detail-sidebar-sticky">
                    ...
                    <nav className="detail-nav-elite">
                        {sections.map(s => (
                            <button
                                key={s.id}
                                className={`nav-item-btn ${activeSection === s.id ? 'active' : ''} ${s.hideOnMobile ? 'hide-on-mobile' : ''}`}
                                ...
                                {readinessScore === null ? (
                                    <div className="assessment-intro">
                                        ...
                                        <div className="quiz-container-mock">
                                                ...
                                                <div key={i} className="quiz-question-card animate-in" style={{ animationDelay: `${i * 0.1}s` }}>
                                                    ...
                                                    <div className="quiz-options-mock">
                                                        {q.options.map((opt, oi) => (
                                                            <div 
                                                                key={oi} 
                                                                className={`opt-mock ${answers[i] === oi ? 'active' : ''}`}
                                                                onClick={() => setAnswers(prev => ({ ...prev, [i]: oi }))}
                                                            >
                                                                <div className="opt-letter-badge">{String.fromCharCode(65 + oi)}</div>
                                            ...
                                            <button className="calculate-score-btn" onClick={calculateActualScore}>
                                    ...
                                    <div className="readiness-result-panel animate-slide-up">
                                        ...
                                        <div className="readiness-feedback">
                                            ...
                                            <button className="back-to-dashboard" style={{ width: 'auto', marginInline: 'auto' }} onClick={() => { setReadinessScore(null); setAnswers({}); }}>
