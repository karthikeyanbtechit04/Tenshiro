    ...
    return (
        <div className="apt-container">
            <style>{`
                ...
                .apt-nav-tabs {
                    display: flex; gap: 2rem; overflow-x: auto; padding-bottom: 5px;
                }
                .apt-content { max-width: 1300px; margin: 0 auto; padding: 2rem; }
                
                /* Layout Engine Classes */
                .apt-hero-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 1rem; }
                .apt-gamification-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.25rem; }
                .apt-sh-grid { display: grid; grid-template-columns: 1fr 400px; gap: 2rem; margin-top: 2.5rem; }
                
                @media (max-width: 1024px) {
                    .apt-sh-grid { grid-template-columns: 1fr; }
                    .apt-gamification-grid { grid-template-columns: repeat(2, 1fr); }
                }

                @media (max-width: 768px) {
                    .apt-navbar { padding: 1rem; flex-direction: column; gap: 1rem; align-items: flex-start; }
                    .apt-nav-tabs { gap: 1rem; width: 100%; white-space: nowrap; }
                    .apt-content { padding: 1rem; }
                    
                    .apt-hero-grid { grid-template-columns: repeat(3, 1fr); gap: 0.75rem; }
                    .apt-gamification-grid { grid-template-columns: 1fr; }
                }
                
                @media (max-width: 480px) {
                    .apt-hero-grid { grid-template-columns: repeat(2, 1fr); }
                }
            ...
            {view === 'main' && (
                <>
                    <nav className="apt-navbar">
                        ...
                        <div className="apt-nav-tabs">
                            {PIPELINE_STEPS.map(step => (
                                <button key={step.id} onClick={() => setActiveTab(step.id)}
                                    style={{ border: 'none', background: 'transparent', color: activeTab === step.id ? '#6366f1' : 'var(--text-muted)', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.2s', flexShrink: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                            <div style={{ textAlign: 'right' }}>
                                ...
                                <div style={{ fontSize: '1.1rem', fontWeight: 900, color: '#1A3263' }}>{readiness}%</div>
                            </div>
